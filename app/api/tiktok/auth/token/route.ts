...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      const account = await handleTikTokOAuthCallback(code, state, codeVerifier, redirectUri);

      return NextResponse.json({ account }, { status: 200 });
    } catch (error: any) {
      console.error("TikTok API token exchange - Error in handleTikTokOAuthCallback:", error);
      ...
      if (rootCause.includes("Authorization code is expired")) {
        ...
        return NextResponse.json(
          { 
            ...
            details: "Authorization codes from TikTok can only be used once and expire quickly. This error typically happens if you refresh the page after authorizing or if there's a delay in processing.",

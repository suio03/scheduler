...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.redirect(new URL('/settings/accounts?success=connected', req.url));
  } catch (error: any) {
    console.error('Error handling TikTok OAuth callback:', error);
    
    // Redirect to the accounts page with error message
    return NextResponse.redirect(
      new URL(`/settings/accounts?error=${encodeURIComponent(error.message || 'unknown_error')}`, req.url)
    );
  }
}
